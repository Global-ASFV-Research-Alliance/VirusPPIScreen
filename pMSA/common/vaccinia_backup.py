...
def generate_pMSA(proteinA_msa, proteinB_msa, output_dir="", paired_tr=.99, unpaired_tr=.90, temp='/home/jacob.fenster/tmp/',  hhfilter='hhsuite/bin/hhfilter'):
...
def sys_jackhmmer_search(fasta_query, seq_db, j_out, threads=1, iterations=3, jhmmer="hhsuite/jackhmmer"):
    # this def conducts a jackhmmer search using os.system() command 
    # returns the file path to the search result 
    # fasta_query - single protein query in fasta format. seq_db - fasta format database for search. j_out - where _jhmmer.sto files are saved
    # 
    basename = os.path.basename(fasta_query).split('.')[0]
    jhmmer_sto = f"{j_out}{basename}_jhmmer.sto"
    os.system(f'{jhmmer} -N {iterations} --cpu {threads} --notextw -A {jhmmer_sto} {fasta_query} {seq_db}')
    return jhmmer_sto
...
def sys_gen_seed_hmmalign(query_fasta, descs, seqs, temp='home/jacob.fenster/temp/', phmmer='phmmer', hmmbuild='hmmbuild', hmmalign='hmmalign', reformat='hhsuite/scripts/remformat.pl'):
    ...
    bpar.fasta_out_single_line(descs, seqs, f"{temp}{query_desc}-unaligned.fasta")
    # generate phmmer search of unaligned sequences
    os.system(f"{phmmer} --notextw -A {temp}{query_desc}-phmmer.sto {query_fasta} {temp}{query_desc}-unaligned.fasta") 
    # generate the seed alignments from this search 
    sto_descs, sto_seqs = bpar.read_hmmer_sto(f"{temp}{query_desc}-phmmer.sto") 
    phmmer_query_seq = sto_seqs[0]
    duplicates = find_duplicates(sto_descs, sto_seqs)
    ...
    seed_sto_out(df, f"{temp}{query_desc}-seed.sto")
    # convert seed alignment to hmm
    os.system(f"{hmmbuild} {temp}{query_desc}-seed.hmm {temp}{query_desc}-seed.sto")
    # align with seed hmm
    os.system(f"{hmmalign} -o {temp}{query_desc}-hmmalign.sto {temp}{query_desc}-seed.hmm {temp}{query_desc}-unaligned.fasta")
    # clean up the alginment 
    aln_descs, aln_seqs = bpar.read_hmmer_sto(f"{temp}{query_desc}-hmmalign.sto")
...
def assemble_paired_unpaired_pMSA(pmsa_descs, pmsa_seqs, protA_descs, protA_seqs, protB_descs, protB_seqs, protA_len, protB_len):
    ...
    master_descs, master_seqs = [], []
    master_descs.extend(pmsa_descs), master_seqs.extend(pmsa_seqs)
    for desc, seq in zip(protA_descs, protA_descs):
        master_descs.append(desc)
        master_seqs.append(f"{seq}{'-' * protB_len}")
    for desc, seq in zip(protB_descs, protB_descs):
        master_descs.append(desc)
        master_seqs.append(f"{'-' * protA_len}{seq}")
    return master_descs, master_seqs
...
def generate_pMSA(proteinA_msa, proteinB_msa, output_dir="", paired_tr=.99, unpaired_tr=.90, temp='/home/jacob.fenster/tmp/',  hhfilter='hhsuite/bin/hhfilter'):
    ...
    protA_name, protB_name = labA[0], labB[0] # assumes first sequece is query which is the protein name
    pmsa_name = f"{protA_name}-AA{protA_len}__{protB_name}-AA{protB_len}"
    pmsa, uniqueA, uniqueB = pMSA_dict(dictA, dictB)
    # filter each dictionary
    msa_dict_to_a3m(pmsa, f"{temp}{pmsa_name}.a3m"),  msa_dict_to_a3m(uniqueA, f"{temp}{protA_name}-unique.a3m"), msa_dict_to_a3m(uniqueB, f"{temp}{protB_name}-unique.a3m")
    os.system(f"{hhfilter} -v 0 -id {paired_tr} -i {temp}{pmsa_name}.a3m -o {temp}{pmsa_name}-fil.a3m")
    os.system(f"{hhfilter} -v 0 -id {unpaired_tr} -i {temp}{protA_name}-unique.a3m -o {temp}{protA_name}-fil.a3m")
    os.system(f"{hhfilter} -v 0 -id {unpaired_tr} -i {temp}{protB_name}-unique.a3m -o {temp}{protB_name}-fil.a3m")
    # assemble paired-unpaired pMSA 
    pmsa_descs, pmsa_seqs = bpar.read_fasta(f"{temp}{pmsa_name}-fil.a3m")
    protA_descs, protA_seqs = bpar.read_fasta(f"{temp}{protA_name}-fil.a3m")
    protB_descs, protB_seqs = bpar.read_fasta(f"{temp}{protB_name}-fil.a3m")
    pu_descs, pu_seqs = assemble_paired_unpaired_pMSA(pmsa_descs, pmsa_seqs, protA_descs, protA_descs, protB_descs, protB_seqs, protA_len, protB_len)
    # ouptut paired and paired_unpaired pMSAs 
    os.makedirs(output_dir, exist_ok=true), os.makedirs(f"{output_dir}paired/", exist_ok=True), os.makedirs(f"{output_dir}paired_unpaired/", exist_ok=True)
    bpar.fasta_out_single_line(pu_descs, pu_seqs, f"{output_dir}paired_unpaired/{pmsa_name}-pu.a3m")
    bpar.fasta_out_single_line(pmsa_descs, pmsa_seqs, f"{output_dir}paired/{pmsa_name}-p.a3m")
    return pmsa_descs, pmsa_seqs, protA_descs, protA_seqs, protB_descs, protB_seqs
...
if True:
    for query_fasta in glob.glob(f"{query_fasta_dir}*.fasta"):
        ...
        protein_name = os.path.basename(jhmmer_file).split('_jhmmer.sto')[0]
...
if False:
    for pair in pairs:
        ...
        protinB_msa = f"{combined_msa_out}{pair[1]}.fasta"
        # generate paired and unpaired pMSAs and ouptut to .a3m files. This returns the raw data for downstream stats
        pmsa_descs, pmsa_seqs, protA_descs, protA_seqs, protB_descs, protB_seq =  pmsa.generate_pMSA(proteinA_msa, proteinB_msa, output_dir="", paired_tr=.99, unpaired_tr=.90, temp='/home/jacob.fenster/tmp/',  hhfilter=hhfilter)
