...
in_MSA_dir, output_dir, fasta, chain_map_parse = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
...
for dir in MSA_dirs:
    with open(f"{dir}msas/chain_id_map.json", 'r') as f:
        data = json.load(f)
    # adding chaing map parse for different formats
    if chain_map_parse == 'default':
        protA, protB = data['A']['description'], data['B']['description']
    if chain_map_parse == 'homodimer':
        protA, protB = data['A']['description'].split('__')[0], data['B']['description'].split('__')[1]
    if chain_map_parse == 'vacc_homodimer':
        protA, protB = data['A']['description'].split('.')[0], data['B']['description'].split('.')[0]
...
if os.path.exists(f"{output_dir}{fasta_basename}/msas/A/") and os.path.exists(f"{output_dir}{fasta_basename}/msas/B/"):
    ...
    exit_status = 0
if exit_status == 1:
    os.system(f"rm -r {output_dir}{fasta_basename}/")
